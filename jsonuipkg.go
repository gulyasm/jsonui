...
// Most of the code written here is from a program called jsonui  https://github.com/gulyasm/jsonui -- I did not write this
// but modified it to work in xjconvert. -- rx7322.
...
const (
	treeView = "Hit 'h' for help, 'x' execute."
	textView = "text"
	pathView = "path"
	helpView = "help"
)
...
func startInteract(jsonBod []byte) (jp string) {
	var err error
	tree, err = fromBytes(jsonBod)
	if err != nil {
		log.Panicln(err)
	}
	...
	if err := g.SetKeybinding("", 'x', gocui.ModNone, quit); err != nil {
		log.Panicln(err)
	}
